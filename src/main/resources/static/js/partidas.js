    ...
    fetch('/api/partidas')
      ...
      .then(partidas => {
        ...
        cont.innerHTML = partidas.map(p => {
          const creadorHtml = p.creador
            ? `<a href="/perfil_publico?id=${p.creador.id}" class="enlace-creador text-primary" style="text-decoration:underline;">${p.creador.nombreUsuario}</a>`
            : 'Anónimo';

          return `
            <div class="card mb-3 foro-card partida-card" data-id="${p.id}" style="cursor:pointer;">
              <div class="card-body">
                <span class="badge bg-primary mb-2">Partida</span>
                <h5 class="card-title">${p.titulo}</h5>
                <p class="mb-1"><strong>Creador:</strong> ${creadorHtml}</p>
                <p class="mb-1"><strong>Descripción:</strong> <p class="card-text">${p.descripcion}</p>
                <p class="mb-1">
        ...
        document.querySelectorAll('.partida-card').forEach(card => {
          card.addEventListener('click', function(e) {
            if (e.target.closest('a')) return;
            window.location.href = `/partida_detail?partidaId=${card.dataset.id}`;
          });
        });
      });
  }

        ...
        cont.innerHTML = `
          ...
          <div class="my-3">
            <p><strong>Director:</strong> ${
              p.creador
                ? `<a href="/perfil_publico?id=${p.creador.id}" class="text-primary text-decoration-underline">${p.creador.nombreUsuario}</a>`
                : 'Anónimo'
            }</p>
            ...
            <ul id="lista-jugadores" class="list-group">
              ${(p.jugadores || []).map(j =>
                `<li class="list-group-item">
                  <a href="/perfil_publico?id=${j.id}" class="text-primary text-decoration-underline">${j.nombreUsuario}</a>
                </li>`
              ).join('')}
            </ul>
          </div>
